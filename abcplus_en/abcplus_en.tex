...
% if the PDF is meant to be read on a computer screen:
% \usepackage[margin=2.6cm]{geometry}
\usepackage[outer=2.2cm,inner=3cm,top=3cm,bottom=2.6cm]{geometry}
\usepackage{setspace} 
...
\begin{titlepage}
...

  {
    ...
    \color{textcovercolor}
     
    \resizebox{!}{1.2cm}{Making Music with}
    
    \vspace{0.5cm}
    
    \resizebox{!}{1.6cm}{\ABCPLUS{}}
    
    \vspace{0.6cm}
    
    \begin{margins}{-2.2cm}{-3cm}
      \makebox[\linewidth]{\includegraphics[width=\linewidth]{music.jpg}}
    \end{margins}

    \vspace{0.6cm}
    
    \resizebox{!}{0.65cm}{\textit{A practical guide to modern \ABC{}
    notation}}
    
    \vspace{1.5cm}
    \resizebox{!}{0.55cm}{by Guido Gonzato}
...
% \begin{center}
%   \large{THIS IS A WORK-IN-PROGRESS (\today) RELEASE.}
% \end{center}
...
% theoretical standard, while \ABCPLUS{} is what you can actually do
% with available programs. It should be emphasised that \ABCPLUS{} is
% fully upwards compatible with \ABC; in the future, these two
% ``releases'' of the same music notation language might even merge.
...
Windows 7 and later: start the menu then insert \cmd{cmd} in ``Search
Programs and Files''.)
...
To display the \ps{} output, GNU/Linux users have a wide choice of
viewers; use \cmd{xdg-open} if you're in doubt. Mac OS X has a Preview
utility that will automatically open \file{scale1.ps} and convert it
into PDF format.

\begin{alltt}
~/abcmusic$ \textbf{xdg-open scale1.ps &}
\end{alltt}
...
\paragraph{Android (expert users only!)}

To use \ABC{} software on Android, you will have to install a terminal
emulator beforehand. Please refer to the instructions available at the
\ABCPLUS{} home page.

Android users will follow almost the same procedure as GNU/Linux
users. The difference is the lack of Ghostscript for the Android
platform: you will have to use SVG output instead.

Once your \ABC{} source is ready, use this command line:

\begin{alltt}
~/abcmusic$ \textbf{abcm2ps -c -O= -X scale1.abc}
abcm2ps-7.8.14 (March 25, 2015)
File scale1.abc
...
please note the \cmd{-X} switch: it tells \abcm{} to produce SVG
output in a file called \file{scale1.xhtml}. To browse this file, just
...
In addition, \field{L:} may contain a ``keyword'', \field{auto}. In
...
Another weird note is the first \car{E} in measure 5. The trailing
\car{0} denotes a stemless quarter note, which is sometimes found in
early music. \devel{} The same effect may obtained with the
\cmd{!stemless!} \emph{ornament} preceding the note. Ornaments will be
...
\car{\char94} or \car{\_}, while 3/4 are obtained adding \cmd{3/2}:
...
% TO DO: %%microscale + %%micronewps in 8.*
...
ISO--8859, and many others. A \file{.abc} file encoded in, say,
...
\cmd{\textbackslash{}U0024} produces \car{\textdollar}.
...
(\cmd{w:}) may be broken into several lines in the same manner. The
first part of a music line that ends in \car{\bl} must be followed by
the corresponding \field{w:} line, if any.
...
%% This section will be written when the bugs in this features are fixed.

% \subsection{Repeated Sequences}
% \label{sec:repeats}

% Now that we know inline fields, let's start using them.

% Sometimes, we want to repeat a sequence of notes (or a bar) several
% times; this can be done with the inline field \field{I: repeat}. It
% can also specify one or two numbers, 
...
\field{F\#m}. When there are no accidentals, the keyword \field{none}
may also be used.
...
\optparm{line number} \optparm{+8} \optparm{-8} \optparm{transpose=}\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item the kwyword \field{none} indicates that there is no clef;
    
    \item the keyword \field{perc} (or, equivalently, the letter
    \car{p}) indicates a clef for percussion instruments.
  ...
  \item \field{\textasciicircum{}8} and \field{\_8} draws `8' above
...
different clefs using the \field{octave} field:
...
\begin{itemize}
  ...
  indications are also supported: \field{M: o} (perfect minor),
  \field{M: o.} (perfect major), \field{M: c} (imperfect minor),
  \field{M: c.} (imperfect major);
...
example) may be omitted. Besides, \field{Q:} also accepts references
to previous tempos, as in \field{Q: 3/8 = 1/4}.
...
\begin{warn}
  ...
  In multivoice tunes, the \field{P:} field must be indicated in the
  top voice.
...
\subsection{Positioning Voices: \icmd{\%\%score}}
\label{sec:score}
...
score, you use the \metacmd{score} command.

The \metacmd{score} command must be followed by voice names,
optionally enclosed by a pair of delimiters: \field{[]}, \field{\{\}},
and \field{()}. As other commands in the header, \metacmd{score} must
...
\begin{itemize}
  ...
  will be the first voice in the list. For example: \metacmd{score
  SATB}
  ...
  used for the choral part of a system. For example: \metacmd{score
  [SATB]}
  ...
  the piano or organ part of a system: \metacmd{score \{MS MD\}}
  
  \item if two or more voices are enclosed between parentheses,
  they will be printed on the same staff. For example: \metacmd{score
  [(SA) (TB)]}
  
  \item to add measure bars that cross the staves, use the character
  \car{\textbar} between voice names: \metacmd{score [S|A|T|B]}
...
code of the \car{!`} character: we will cover this topic in
...
accomplished using the \cmd{I: voicescale} command, as in the
following example:
...
\metacmd{beginsvg}{\ldots} \metacmd{endsvg}.
...
\file{Holla.\-zip}. Obviously, the procedure will be very similar with
other fonts.

Unzip the archive and copy \file{HollaScript.pfb} to the directory
...
\path{/var/lib/ghostscript/fonts/Fontmap}. Move to the bottom of
...
which defines a new font called \file{HollaScript}. If you wish, you
...
Another way to play chords as arpeggios is the \metacmd{MIDI
chordattack} command. When followed by a number \parm{n} greater than
0, it delays each note by \parm{n} MIDI time units (a quarter note is
480 MIDI units). Using \metacmd{MIDI randomchordattack} \parm{n}
varies the delay of each note randomly between 0 and \parm{n}.
...
% \section{Using the \cmd{abcm2ps} Command}
% \label{sec:abcm2ps}

% To some extent, even \abcm{} can be used as a preprocessor.
...
I started with \ABC{} several years ago, and it's always been my tool
of choice whenever I needed to make a score. This manual started out
as a list of notes I wrote for myself, and it slowly grew to its
present form. Since the \ABC{} software is free, I decided it was only
fair to release this manual for free as well.
...
\begin{description}
  ...
  footer on each page; see Section~\ref{sec:headerfooter} for details.

  \item[\metacmd{header} \parm{text}:] sets the text to be printed as
  header on each page; see Section~\ref{sec:headerfooter} for details.
...
\end{verbatim}
  ...
  option to be used between \metacmd{be\-gin\-text} and
  \metacmd{end\-text}, or in \metacmd{EPS} files. The parameter can be
  a digit or a corresponding string: 0 or \cmd{obeylines}), 1
  (\cmd{justify}), 2 (\cmd{fill}), 3 (\cmd{center}), 4 (\cmd{skip}), 5
  (\cmd{right}). If the option is 4 (\cmd{skip}), no text or EPS is
  output.
...
% TO DO - inconsistent!
  ...
  Section~\ref{sec:titles} for examples.
...
\begin{description}
  ...
  \item[\metacmd{barsperstaff} \parm{int}:] attempts to typeset the
  score with \parm{int} bars on each line.

  \item[\metacmd{breaklimit} \parm{float}:] used together with
  \metacmd{maxshrink}, it lets the user control where line breaks may
  occur. The parameter can range between 0.5 (line break occurs when
  the line is 50\% full) and 1.0.
  ...
  \item[\metacmd{linebreak} \parm{string}:] defines the line break
  separators. The string may be empty or contain the values
  \field{\textless{}EOL\textgreater{}}, \field{\$}, \field{!}, and
  \field{\textless{}none\textgreater{}}. The latter option (or an
  empty string) computes line breaks automatically, as the deprecated
  command \metacmd{continueall}.
  ...
  \item[\metacmd{voicescale} \parm{len\-gth}:] sets the scale of a
  voice, or of all voices if present in the header.
...
\begin{description}
  ...
  \item[\metacmd{abcm2ps} \parm{char}]: changes the character that
  defines a pseudo-comment.
  ...
  \item[\metacmd{beginps}--\metacmd{endps}:] starts/ends a sequence of 
  \ps{} commands; please see Section~\ref{sec:customps}.
  
  \item[\metacmd{beginsvg}--\metacmd{endsvg}:] starts/ends a sequence of 
  SVG commands; please see Sec\-tion \ref{sec:customps}.
  
  % \item[\metacmd{bgcolor}:] TO BE WRITTEN.
  ...
  \item[\metacmd{cancelkey} \parm{logical}:] if true, accidentals are
  cancelled using natural signs when the key signature changes.
  
  \item[\metacmd{clef} \parm{clef} \optparm{line} \optparm{octave}:]
  inserts a clef change, in a manner similar to \field{K:}.
  \parm{clef} may be \cmd{treble} (or \cmd{G2}), \cmd{alto}
  (\cmd{C3}), \cmd{tenor} (\cmd{Cf}), \cmd{bass} (\cmd{F4}),
  \cmd{perc} (\cmd{P2}), \cmd{G}, \cmd{C}, \cmd{G}, \cmd{C}, \cmd{F},
  \cmd{P}, or \cmd{none}. The latter prints no clef on the staff. The
  name of a user-provided \ps{} glyph may also be used. \optparm{line}
  specifies the staff line on which the clef sits. \optparm{octave}
  can be \cmd{+8} or \cmd{-8}, to print `8' above or below the clef;
  \cmd{\textasciicircum{}8} or \cmd{\_8}, to print `8' above or below
  the clef and perform octave transposition.
  ...
  This command is deprecated; use \metacmd{linebreak none} and
  \metacmd{linewarn 0} instead.
  
  \item[\metacmd{custos} \parm{logical}:] if true, a custos is added
  at the end of each music line. A custos is a symbol that indicates
  the pitch for the first note of the next line. This command works
  with single voice tunes only.
  
  \item[\metacmd{dateformat} \parm{string}:] defines the format of
  date and time. Default is \cmd{\%b \%e, \%Y \%H:\%M}. The fields
  specify, respectively: abbreviated month name (Jan--Dec), day of
  month (1--31), year, hour (0--23), minute (0--59).

  \item[\metacmd{deco} \parm{string1} \parm{int1} \parm{string2} 
  \parm{int2} \parm{int3} \parm{int4} \parm{string3}:] adds a new
  decoration. Details are explained in Section~\ref{sec:decopers}.

  \item[\metacmd{decoration} \parm{char}:] defines the decoration
  separator, which can be \car{!} (default) or \car{+}.

  \item[\metacmd{dblrepbar} \parm{string}:] defines how double repeat
  bars are drawn; e.g. \cmd{:][:} (default), or \cmd{::}, or
  \cmd{:|:}.
  
  \item[\metacmd{dynalign} \parm{logical}:] if true, horizontally 
  aligns the dynamics marks.

  \item[\metacmd{dynamic} \parm{int}:] sets the position of
  dynamics (e.g. crescendo, etc.). The parameter may be `0' (or
  ``auto'') for automatic position; `1' or ``above'', `2' or
  ``below'', `3' or ``hidden'' to hide dynamics marks.
  % TO DO: scope.
  ...
  \item[\metacmd{glyph} \parm{string1} \parm{string1}:] sets the name
  of a Unicode glyph. The first parameter is a hexadecimal value, the
  second is the name of the associated glyph in the font files.
  
  \item[\metacmd{gstemdir} \parm{int}:] sets the position of grace
  notes stems. The parameter may be `0' (or ``auto'') for automatic
  position; `1' or ``above'', `2' or ``below'', `3' or ``hidden'' to
  hide dynamics marks.
  % TO DO: scope.
  ...
  \item[\metacmd{keywarn} \parm{logical}:] if true, a cautionary key
  signature is printed when a key signature change occurs at the
  beginning of a music line.
  
  \item[\metacmd{linewarn} \parm{logical}:] if true, outputs a warning
  message if there are too many or too few elements in a music line.

  \item[\metacmd{measurenb} \parm{int}:] draws the measure number
  every \parm{int} bars.

  \item[\metacmd{measurebox} \parm{logical}:] if true, draws a box
  around measure numbers.

  \item[\metacmd{measurefirst} \parm{int}:] starts numbering the
  measures from \parm{int}. This command is obsolete and should be
  replaced by \metacmd{setbarnb}.

  \item[\metacmd{micronewps} \parm{logical}:] if true, uses the new
  PostScript function to draw microtonal accidentals.

  \item[\metacmd{musiconly} \parm{logical}:] if true, lyrics are
  not output.

  \item[\metacmd{oneperpage} \parm{logical}:] outputs one tune per page.

  \item[\metacmd{ornament} \parm{int}:] sets the position of ornaments
  (e.g. trill, etc.). The parameter may be `0' (or ``auto'') for
  automatic position; `1' or ``above'', `2' or ``below'', `3' or
  ``hidden'' to hide ornaments.
  % TO DO: scope.
  
  \item[\metacmd{pango} \parm{logical}:] if \abcm{} was compiled with
  Pango support, this command enables or disables \ps{} output with
  Pango. % TO BE EXPANDED

  \item[\metacmd{partsbox} \parm{logical}:] draws a box around part names.

  \item[\metacmd{pdfmark} \parm{int}:] if the parameter is 1,
  inserts marks in the \ps{} output for titles. \ps{} to PDF
  translators can use the marks to create a PDF tune index. If the
  parameter is greater than 1, marks are inserted for titles and
  subtitles.
  ...
  % (\field{Q:}). SEE WRITEFIELDS
  
  % REMOVED
  % \item[\metacmd{pslevel} \parm{int}:] specifies the \ps{} level (1, 2,
  % or 3).
  
  \item[\metacmd{ps} \parm{string}:] same as \metacmd{postscript}.
  
  \item[\metacmd{repbra} \parm{logical}:] if false, prevents
  displaying repeat brackets for the current voice.
  
  \item[\metacmd{repeat} \parm{int1} \parm{int2}:] repeats a sequence
  of notes or bars. % ; please see Section~\ref{sec:repeats} for details.
  % TO DO: wait for the bugfix
  
  \item[\metacmd{score} \parm{string}:] defines how staves are to be
  printed. Please see Section~\ref{sec:score} for details.
  
  \item[\metacmd{setbarnb} \parm{int}:] sets the number of the next
  measure, excluding the first. To set the number of the first
  measure, the command must be written in the tune header.
  ...
  \item[\metacmd{shiftunison} \parm{logical}:] if true, shifts note heads
  ...
  \item[\metacmd{stafflines} \parm{int}:] sets the number of staff lines
  of the current voice; default is 5.
  
  \item[\metacmd{staffnonote} \parm{int}:] avoids printing a staff
  according to the parameter. If it's 0, the staff is not printed if it
  contains only invisible rests and notes; if it's 1, the staff is not
  printed if it contains only rests and invisible notes; if it's 2,
  the staff is printed.
  
  \item[\metacmd{staffscale} \parm{float}:] sets the scale of the staff
  of the current voice.
  
  \item[\metacmd{staves} \parm{string}:] defines how staves are to be
  printed. This command is deprecated; please use \cmd{score} instead.
  Please see Section~\ref{sec:score} for details. 
  
  \item[\metacmd{stemdir} \parm{int}:] sets the note stems directions.
  The parameter may be `0' (or ``auto'') for automatic position; `1'
  or ``up'', `2' or ``down''.
  % TO DO: scope.
  ...
  \item[\metacmd{tablature}:] defines a tablature. Please see
  Section~{sec:tablatures}.
  ...
  \item[\metacmd{transpose} \parm{int} \optparm{char}:] transposes the
  music by \parm{int} semitones. The optional parameter may be
  \car{\#} or \car{b}, meaning that the new key signature will have
  sharps or flats.
  % TO DO: add a section on transposition.

  \item[\metacmd{tuplets} \parm{int1} \parm{int2} \parm{int3}:]
  defines how tuplets are to be drawn. See Section~\ref{sec:tuplets} for
  details.

  \item[\metacmd{user} \parm{string}:] this command is the equivalent
  of the \field{U:} field, and can be used to redefine default
  decorations in format files.

  \item[\metacmd{vocal} \parm{int}:] sets the position of lyrics. The
  parameter may be `0' (or ``auto'') for automatic position; `1' or
  ``above'', `2' or ``below'', `3' or ``hidden'' to hide lyrics.
  
  \item[\metacmd{vocalabove} \parm{logical}:] draws the vocals above the
  staff. This command is obsolete and should be replaced by
  \metacmd{vocal}.

  \item[\metacmd{volume} \parm{int}:] sets the position of volume
  decorations (\cmd{!f!}, etc.). The parameter may be `0' (or
  ``auto'') for automatic position; `1' or ``above'', `2' or
  ``below'', `3' or ``hidden'' to hide the decorations.
  % TO DO: scope.
  ...
  \item[\metacmd{writefields} \parm{string} \parm{logical}:] if true,
  displays the information fields defined in \parm{string}.
...
% this line raises hell! fonts.pdf must be rebuilt.
